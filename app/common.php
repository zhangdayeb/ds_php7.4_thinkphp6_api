...
function getUnameById($agentID){
    if($agentID > 0){
        $info = (new AdminModel)->find($agentID);
...
function addOneAgentLink($agentUid,$code){
    $promo = Db::name('common_spread')->field('id,url,title')->select()->toArray();// 获取所有推广链接

    $spreadAgentModel = new SpreadAgent();// 每一个代理的 链接 地址
    $dwzService = new DwzkrService(); // 链接打包接口
    ...
    $spreadAgentData = [
        ...
        'qq_url' => '',
            ...
            // 遍历所有的 推广链接地址  | 然后 根据 普通 微信 抖音 QQ 微博 进行 设置 
            // 普通的
            $spreadId = $promo[0]['id'];
            $spreadUrl = $promo[0]['url'];
            $longUrl = $spreadUrl . '/code/' . $code; // 从 ?code=123456 改成 /code/123456 | 防止链接打包不行 
            // echo 'spreadId:' . $spreadId . PHP_EOL;
            // echo 'agentUid:' . $agentUid . PHP_EOL;
            // echo 'longUrl:' . $longUrl . PHP_EOL;
            $dwzUrls = $dwzService->get_dwzkr_urls($longUrl);
            // echo 'dwz:' . var_export($dwzUrls, true) . PHP_EOL;
            // echo '======================================' . PHP_EOL;
            $spreadAgentData['sms_url'] = $dwzUrls['sms_url'] ?? '';
            $spreadAgentData['custom_url'] = $dwzUrls['custom_url'] ?? '';
            $spreadAgentData['url_key_web'] = $dwzUrls['url_key'] ?? '';


            // 遍历所有的 推广链接地址  | 然后 根据 普通 微信 抖音 QQ 微博 进行 设置 
            // 微信
            $spreadId = $promo[1]['id'];
            $spreadUrl = $promo[1]['url'];
            $longUrl = $spreadUrl . '/code/' . $code; // 从 ?code=123456 改成 /code/123456 | 防止链接打包不行 
            // echo 'spreadId:' . $spreadId . PHP_EOL;
            // echo 'agentUid:' . $agentUid . PHP_EOL;
            // echo 'longUrl:' . $longUrl . PHP_EOL;
            ...
            // echo 'dwz:' . var_export($dwzUrls, true) . PHP_EOL;
            // echo '======================================' . PHP_EOL;
            $spreadAgentData['wx_url'] = $dwzUrls['wx_url'] ?? '';
            $spreadAgentData['wx1_url'] = $dwzUrls['wx1_url'] ?? '';
            $spreadAgentData['wx2_url'] = $dwzUrls['wx2_url'] ?? '';
            $spreadAgentData['wx3_url'] = $dwzUrls['wx3_url'] ?? '';
            $spreadAgentData['url_key_wx'] = $dwzUrls['url_key'] ?? '';


            // 遍历所有的 推广链接地址  | 然后 根据 普通 微信 抖音 QQ 微博 进行 设置 
            // QQ
            $spreadId = $promo[2]['id'];
            $spreadUrl = $promo[2]['url'];
            $longUrl = $spreadUrl . '/code/' . $code; // 从 ?code=123456 改成 /code/123456 | 防止链接打包不行 
            // echo 'spreadId:' . $spreadId . PHP_EOL;
            // echo 'agentUid:' . $agentUid . PHP_EOL;
            // echo 'longUrl:' . $longUrl . PHP_EOL;
            ...
            // echo 'dwz:' . var_export($dwzUrls, true) . PHP_EOL;
            // echo '======================================' . PHP_EOL;
            $spreadAgentData['qq_url'] = $dwzUrls['qq_url'] ?? '';
            $spreadAgentData['url_key_qq'] = $dwzUrls['url_key'] ?? '';


            // 遍历所有的 推广链接地址  | 然后 根据 普通 微信 抖音 QQ 微博 进行 设置 
            // 抖音
            $spreadId = $promo[3]['id'];
            $spreadUrl = $promo[3]['url'];
            $title = $promo[0]['title'];
            $longUrl = $spreadUrl . '/code/' . $code; // 从 ?code=123456 改成 /code/123456 | 防止链接打包不行 
            // echo 'spreadId:' . $spreadId . PHP_EOL;
            // echo 'agentUid:' . $agentUid . PHP_EOL;
            ...
            // echo 'dwz:' . var_export($dwzUrls, true) . PHP_EOL;
            // echo '======================================' . PHP_EOL;
            $spreadAgentData['dy_url'] = $dwzUrls['dy_url'] ?? '';
            $spreadAgentData['dy1_url'] = $dwzUrls['dy1_url'] ?? '';
            $spreadAgentData['du_url'] = $dwzUrls['du_url'] ?? '';
            $spreadAgentData['du1_url'] = $dwzUrls['du1_url'] ?? '';
            $spreadAgentData['url_key_dy'] = $dwzUrls['url_key'] ?? '';


            // 遍历所有的 推广链接地址  | 然后 根据 普通 微信 抖音 QQ 微博 进行 设置 
            // 微博
            $spreadId = $promo[4]['id'];
            $spreadUrl = $promo[4]['url'];
            $title = $promo[0]['title'];
            $longUrl = $spreadUrl . '/code/' . $code; // 从 ?code=123456 改成 /code/123456 | 防止链接打包不行 
            // echo 'spreadId:' . $spreadId . PHP_EOL;
            // echo 'agentUid:' . $agentUid . PHP_EOL;
            ...
            // echo 'dwz:' . var_export($dwzUrls, true) . PHP_EOL;
            // echo '======================================' . PHP_EOL;
            $spreadAgentData['wb_url'] = $dwzUrls['wb_url'] ?? '';
            $spreadAgentData['url_key_wb'] = $dwzUrls['url_key'] ?? '';

            $spreadAgentModel::create($spreadAgentData);
            // echo PHP_EOL;
}
...
function userzm($length, $needAlpha = true)
{
    ...
    function getPhoneById($uid){
        $r = (new UserModel)->where('id',$uid)->value('phone');
        return $r?$r:'无';
    }
    ...
    function getZhiTuiNum($uid){
        $r = (new UserModel)->where('agent_id_1',$uid)->count('id');
        return $r?$r:'';
    }
    ...
    function updateKLine(){
        ...
        // 获取昨日价格 
        $last = (new TouziKLine())
        ->whereTime('show_day', 'between', ['1970-10-1', date('Y-m-d',(time()- 24 * 60 *60))])
        ->order('show_day desc')->find()->toArray();
        if($last){
            $price_open = $last['price_close'];
        }
        // $price_close = $price_open * (100 + $rand)/100; // 默认百分比
        $price_close = $price_open + $rand;
    ...
    function Tongji_recharge_agent($uid, $level,$map_date=[]) {
        return UserModel::getTotalByField($uid, $level,'money_all_recharge',$map_date);
    }

    //下线 提现总额
    function Tongji_withdraw_agent($uid, $level,$map_date=[]) {
        return UserModel::getTotalByField($uid,$level,'money_all_withdraw',$map_date);
    }

    //下线 购买总额
    function Tongji_buy_money_agent($uid, $level,$map_date=[]) {
        return UserModel::getTotalByField($uid, $level,'money_all_buy',$map_date);
    }
    
    //团队股权
    function Tongji_team_has_fund($uid, $level,$map_date=[]) {
        return UserModel::getTotalByField($uid, $level,'money_fund',$map_date);
    }

    //团队人数
    function Tongji_team_has_nums($uid, $level,$map_date=[]) {
        return UserModel::getCountByField($uid, $level,[],$map_date);
    }

    //团队实名人数N级
    function Tongji_team_real_name_nums($uid, $level,$map_date=[]) {
        return UserModel::getCountByField($uid, $level, ['is_real_name' => 1],$map_date);
    }
    ...
    //今日获得佣金
    function Tongji_fanyong_today($uid) {
        return TouziFanyongLog::where(['user_id' => $uid, 'money_type' => 'money_keti'])
            ->whereDay('create_time')->sum('money_amount');
    }

    //今日返佣 健康点
    function Tongji_fanyong_point_today($uid) {
        return TouziFanyongLog::where(['user_id' => $uid, 'money_type' => 'money_point'])
            ->whereDay('create_time')->sum('money_amount');
    }
    ...
    function Tongji_team_has_nums_tody($uid) {
        return UserModel::getRegisterByToday($uid);
    }
