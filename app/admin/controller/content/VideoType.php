...
class VideoType extends Base
{
    ...
    public function index()
    {
        ...
        //查询搜索条件
        $list = $this->model->where(['pid' => 0])->order('sort desc')->paginate(['list_rows' => $limit, 'page' => $page]);
        if (!empty($list))  {
            $list=$list->toArray();
    ...
    {
        //过滤数据
        $postField = 'title,name,thumb_url,pid,sort';
        ...
        $parentDirName = root_path() . '/public/storage/videotemp';
        if (!is_dir($parentDirName . '/' . $post['title'])) {
            mkdir($parentDirName . '/' . $post['title'], 0777, true);
        }
    ...
    {
        //过滤数据
        $postField = 'id,title,name,thumb_url,pid,sort';
        $post = $this->request->only(explode(',', $postField), 'post', null);
